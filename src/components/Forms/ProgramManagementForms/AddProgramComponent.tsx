...
const AddProgramComponent = ({ open, onOpenChange }: { open: boolean, onOpenChange: React.Dispatch<React.SetStateAction<boolean>> }) => {
    ...
    const formSchema = z.object({
        ...
        station: z.string().min(1, "Radio Station is required"),
        image:
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        createProgram({ token, data: formData }).then((res) => {
            setLoading(false);
            onOpenChange(false);
            form.reset({});
            setPreviewImage("");
