...
import { updateRecepient } from "@/actions/recepient";

import { toast } from "sonner";
import useSWR, { mutate } from "swr";
import { fetcher } from "@/actions/swr";
import { cn } from "@/lib/utils";
export default function UpdateRecepientForm({ open, onOpenChange, recepientId }:
    { open: boolean, onOpenChange: React.Dispatch<React.SetStateAction<boolean>>, recepientId: string }) {
    const token = getCookie("token");

    const [loading, setLoading] = useState(false)
    const { data, isLoading, error } = useSWR(open && {
        url: `v1/recepients/${recepientId}`,
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        setLoading(true)
        updateRecepient({ data: values, token, recepientId }).then(() => {
            setLoading(false)
            onOpenChange(false)
    ...
    useEffect(() => {
        if (error) {
            toast.error("Error fetching recepient data", {
                style: {
                    background: "rgba(220, 46, 46, 1)",
                    color: "white",
                    border: "none"
                },
                description: (error as Error).message || "There was a problem fetching the recepient data, please try again later.",
                duration: 5000
            });
        }
    }, [error])
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-h-[100vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle>
                        Update recepient
                        <LoaderCircleIcon className={cn("inline-block animate-spin ml-2 size-4", !isLoading && "hidden")} />
                    </DialogTitle>
                </DialogHeader>
                <Form {...form}>
